...
class FileInfoDialog {
    ...
    constructor() {
        ...
        this.electron.ipcRenderer.on('set-file-info', (event: Electron.IpcRendererEvent, details: any) => {
            this.showFileInfo(details);
        });
        document.addEventListener('keydown', (event: KeyboardEvent) => {
            if (event.code === 'Escape') {
                this.electron.ipcRenderer.send('close-file-info');
            }
        });
        ...
        setTimeout(() => {
            this.electron.ipcRenderer.send('set-height', { window: 'fileInfo', width: document.body.clientWidth, height: document.body.clientHeight });
        }, 200);
    }
